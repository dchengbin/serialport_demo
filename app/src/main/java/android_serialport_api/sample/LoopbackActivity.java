...
public class LoopbackActivity extends SerialPortActivity {
			...
			while (!isInterrupted()) {
				synchronized (mByteReceivedBackSemaphore) {
					...
					try {
						...
						if (mOutputStream != null) {//mOutputStreamLoopBack    mOutputStream
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		Log.i(TAG, " mSerialPortLoopBack:"+mSerialPortLoopBack);
		if (mSerialPortLoopBack != null) {//mSerialPortLoopBack   mSerialPort
...
//		Log.e(TAG, "///receiveValue//:"+receiveValue);
//		
//		synchronized (mByteReceivedBackSemaphore) {
//			int i;
//			for (i = 0; i < size; i++) {
...
//			mSendingThread.interrupt();
	...
	protected void onDataReceivedLoopBack(byte[] buffer, int size) {
		...
		String receiveValueLoopBack = new String(buffer, 0, size);
		Log.e(TAG, "////receiveValueLoopBack//:"+receiveValueLoopBack);
	}
